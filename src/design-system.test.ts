...
function ruleFor(selector: string): string {
  ...
  const match = normalizeCss(stylesheet).match(new RegExp(`${escapedSelector}\\s*\\{([^}]+)\\}`));
...
describe('Luminous Precision design system', () => {
  ...
  it('defines the Figma color, geometry, and radius tokens', () => {
    ...
    expectDeclaration(root, '--primary', '#003b9a');
    expectDeclaration(root, '--workspace-background', '#ffffff');
    expectDeclaration(root, '--sidebar-background', 'rgba(255, 255, 255, 0.66)');
  ...
  it('applies the shell tokens and the required frosted-glass navigation', () => {
    ...
    expectDeclaration(rail, 'background', 'var(--sidebar-background)');
    ...
    expectDeclaration(rail, 'border-radius', '12px');
