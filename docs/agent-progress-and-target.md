...
- **代码已实现**：项目、用例、录制、PRD 覆盖治理、运行记录、跨运行覆盖风险、项目级离线 HTML 报告、受控恢复草稿，以及 BrowserRuntime/Workflow/Recording 和 Planner/Verifier/Reporter 请求的取消协议均已进入持久化模型和桌面调用链；项目资产支持首次空目录快照、只读诊断、受控外部重载和本地修改的受控更新，所有外部目录写入均为计划、revision/编辑态校验、人工确认三段式，普通状态保存不触发目录写入。Fixture V1 已支持 typed 输入/输出、不可变版本、环境/凭据/资源锁声明、项目资产独立快照和用例精确 `fixtureId@version` 绑定；完整配置的 HTTP Fixture 可在主进程受控执行 setup，并在 setup 部分成功、Case 失败、浏览器启动失败或用户取消后尽力 cleanup。HTTP 生命周期只允许同源根路径、`POST/PUT/PATCH/DELETE`、显式状态码和受限非敏感 JSON；不接受 headers、凭据、Cookie、绝对 URL 或变量插值。setup 可显式映射已声明的非敏感顶层 JSON 输出；该值仅存在于当前主进程运行内存，并且只能由当前用例绑定的精确 Fixture 版本、已确认的 `input/select` 单次解析使用，绝不进入响应体 evidence、运行记录、报告、项目资产、Agent plan 或 renderer。脚本信任作为独立的 `studio-data` 本地记录，绑定项目目录、Fixture 精确版本、生命周期、相对路径和声明内容哈希；只有受信任的自包含 `.mjs` 文件在运行时再次通过真实路径与哈希复核后，才会以短时子进程执行。脚本输入和输出遵循受限 JSON 契约，stdout、stderr、上下文和解析值不会进入运行记录、报告、项目资产、Agent plan 或 renderer。`storageState` 可由主进程直接从用户选择的文件导入或从当前真实 BrowserSession 捕获，并本机加密保存；项目与环境只持有逻辑引用，既有引用可原地刷新或显式撤销。启动浏览器前才由主进程解密、校验并注入 Playwright context，缺失、损坏或已过期引用不会退化为未认证会话。信任不足或执行器缺失时，运行会在浏览器启动前安全阻断。已通过的自然语言 Agent Run 可转存为独立、可编辑的 `naturalLanguage` 用例，并保留原始业务意图、发起时的项目/分组/环境，以及待审阅的 V2 结构化 action、定位 fingerprint、风险和运行来源。用例级 `provenance` 统一记录自然语言运行/步骤、录制资产/节点及 PRD 文档或路径；旧 `prdPath` 仅作为兼容投影，运行记录的文档归属优先从新关联解析。`input/select` 只保存 Agent 提供的结构化目标和项目范围凭据或精确 Fixture 输出引用；编辑器必须选择允许的来源，再由用户显式确认。已确认 V2 的 `navigate`、selector `click/input/select`、selector/超时 `wait`、selector `scroll` 和显式 URL/标题/页面文本/公开 DOM 断言均有独立的无模型执行链路，要求真实 Playwright 页面；混合用例按源顺序编排，失败、等待和取消不会继续派发后续步骤。Suite V1 的桌面工作台已支持清单、不可变版本草稿、共享预检、精确 `id@version` 运行、父级取消和 Case 级结果跳转；主进程复用共享 `SuiteRunner` 与既有 Case 执行路径，只持久化真实 Case 运行记录。
- **离线验证已完成**：状态迁移、风险派生、PRD triage、报告脱敏与 HTML 转义、受控取消（含模型请求、规则降级和 Reporter 产物写入边界）、录制视觉比较异常、表格/图表证据完整度、自然语言运行转 V2 foundation、V2 编辑器的确认/撤销/编辑回退、确认 V2 动作与显式断言的真实页面门禁/单次执行/取消/失败停止，以及凭据输入绑定的项目范围解析、运行计划脱敏和解析失败安全降级，Fixture V1 资产/引用完整性、HTTP same-origin/状态码/取消安全边界、setup/cleanup 顺序和结果隔离与浏览器启动前阻断，Fixture 输出映射的响应体不泄露、精确版本/字符串类型门禁、缺失输出的浏览器启动前安全降级，`storageState` 的加密保存、归属隔离、过期/损坏拒绝和浏览器启动前门禁，均由单元测试、类型检查、构建和差异检查覆盖。2026-08-14 的 Wave 0 还以本地受管 Chromium 执行 localhost confirmed Case，验证了真实 PNG 证据和 Suite 取消；它不调用模型，也不访问业务页面。
...
- 完整的 Hybrid Case intent、目标、定位指纹和断言证据 contract；Case 顶层 schema version `2`、初始 version、来源关联和 fixture/flow/baseline 的空版本引用位已完成向后兼容迁移，但真实资产解析尚未开始。当前 `input/select` 可绑定项目范围已保存凭据或当前 Case 精确 Fixture 版本映射出的字符串输出；通用变量、跨 Case 输出和表达式求值仍不在范围。
...
- UI Fixture、多文件脚本依赖、业务环境变量注入与脚本副作用审计；HTTP setup/cleanup、受控的 setup 输出到当前 Case `input/select`，以及受信任自包含 `.mjs` 脚本的内容哈希复核、短时执行、超时/取消终止和受限 JSON 输出已实现，但仍需以真实业务 API 或脚本样本验证语义、幂等性和清理契约。脚本信任不会自动信任新路径、哈希、Fixture 版本或生命周期。
- 六种终态与独立 flaky 标记，替代 `neutral` 的混合语义。
- 隔离浏览器池、10–100 case 真实验收，以及桌面/CLI 完整相同的 Suite 级报告持久化；当前 CLI 与桌面均通过共享调度器执行固定 `suiteId@version`，但受单 BrowserRuntime 会话限制为串行，桌面只持久化真实成员 Case 记录。
...
- CLI 只接受稳定 ID；可重复 `--case-id` 批量执行，并可选用 `--environment-id` 统一覆盖选中用例的环境。也可用 `--suite-id <id@version>` 执行精确 Suite 版本；该模式不接受环境覆盖，先通过共享调度器校验 Case 版本、依赖和资源声明，再由当前单 BrowserRuntime 串行执行。
...
最近一次离线门禁结果（2026-08-14）：47 个测试文件、454 个测试通过；类型检查、renderer/Electron 构建和差异检查均通过。真实 Chromium smoke 独立执行：1 个测试文件、2 个测试通过，范围限于 test-owned localhost fixture、真实 PNG 证据与 Suite 取消。

默认离线门禁不下载或启动浏览器；browser smoke 由显式 `pnpm test:browser-smoke` 运行。GitHub Actions 已配置冻结安装、受管 Chromium + Headless Shell 与这两层验证，但尚未有托管运行结果。真实模型和业务页面验收仍应单独记录输入页面、模型配置、可复现步骤和证据产物。
